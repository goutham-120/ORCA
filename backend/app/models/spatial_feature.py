...
class SpatialFeatureRepository:
    ...
    def create(
        ...
        if self.db.is_postgres:
            ...
            self.db.execute(
                ...
                (
                    ...
                    _time(feature.valid_from),
                    _time(feature.valid_to),
                    _time(feature.fetched_at),
                    timestamp,
                    feature.freshness_status,
                    feature.confidence,
                    json.dumps(feature.quality),
        ...
        if row is None:
            raise RuntimeError(
                "Spatial feature creation did not persist."
            )
...
def _record_from_row(
    ...
    values.setdefault(
        "properties",
        values.pop("properties_json", {}),
    )

    values.setdefault(
        "quality",
        values.pop("quality_json", {}),
    )

    values.setdefault(
        "geometry",
        values.pop("geometry_json", None),
    )
    for key in (
        "properties",
        "quality",
