...
def register(request: RegisterRequest) -> AuthResponse:
    ...
    if role_raw == "marine_&_disaster_operations" or role_raw == "marine_and_disaster_operations":
...
def me(current_user: Annotated[User, Depends(get_current_user)]) -> UserResponse:
    ...
    record_login_activity(
        ...
        role=current_user.role or "fisherman",
    )
    return _response_user(current_user)
...
def update_profile(request: ProfileUpdateRequest, current_user: Annotated[User, Depends(get_current_user)]) -> UserResponse:
    """Update profile user_category without allowing role mutation."""
    if current_user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not authenticated")
    updated = users.update_category(current_user.id, request.user_category)
    if updated is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found.")
    return _response_user(updated)
