...
export default function Login({ navigate }) {
  ...
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(''); try { const result = await login(form); navigate(result.user.user_category ? '/dashboard' : '/personalization') } catch (err) { setError(err.message || 'Unable to sign in. Check that the API is available.') } finally { setBusy(false) } }
